...
Screen_shader::create_line_geometry(const Line_2D& line)
{
    ...
    auto norm = glm::rotate(line.end_pos - line.start_pos,
                            static_cast<float>(PI / 2));
    norm = 0.5f * line.width * glm::normalize(norm);

    Line_array d1, d2, d3, d4;
    d1.vert = line.start_pos + norm;
    d2.vert = line.end_pos   + norm;
    d3.vert = line.end_pos   - norm;
    d4.vert = line.start_pos - norm;
    ...
    geom->data_array.push_back(d3);
    geom->data_array.push_back(d4);
